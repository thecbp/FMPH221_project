...
$$
W_{n}=n g\left(\widehat{\theta}_{n}\right)^{\top}\left[J_{g}\left(\widehat{\theta}_{n}\right) \hat{V}_{n} J_{g}\left(\widehat{\theta}_{n}\right)^{\top}\right]^{-1} g\left(\widehat{\theta}_{n}\right)
$$
...
# Perform the test
T.2.test.B = n * t(C %*% mu.diffs) %*% 
  solve(C %*% var.pooled %*% t(C)) %*%
  (C %*% mu.diffs)

# Calculate the statistic
F.stat.test.B = (n - c)/((n - 1)*c) * T.2.test.B

# Now p-value
pvalue.test.B = 1 - pf(F.stat.test.B, c, n - c)
```
