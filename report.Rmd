...
Given our literature and what was present in our dataset, we aimed to examine how different socioeconomic and demographic factors were associated with cancer mortality on a county level, with a focus on race and education.

### Data Cleaning

A few of the variables in the dataset showed skewed distributions such as `avgAnnCount` and `PctAsian` (Appendix Fig X). Therefore, we log-transformed these variables so that their distributions were closer to normality. Furthermore, we decided to exclude a few counties based on values that were magnitudes beyond any other county or had values that did not make sense (i.e. `MedianAge > 100`). There were also two variables, `PctEmployed16_Over` and `PctPrivateCoverageAlone`, both of which had < 20% missing data. We used the `mice` package in R to impute these missing values. 
...
The change was most drastic in Asians, but the trend held over all races present in the data. For African-Americans, we also see an attenuation of the average cancer mortality despite it not converting to a negative correlation. Our literature review showed that both education and race are both significantly associated with cancer mortality, so it raised an interesting question on if this can be utilized on a county level.
...
The focus of our project is to create a useful predictive model for cancer mortality.
...
- **Jasen**: exploratory analysis for predictive model, handled predicitve modeling, writing for predictive model parts of the report, editing/revising
- **Christian**: exploratory analysis for explanatory model, handled explanatory modeling, writing for explanatory model parts of the report, performed the literature review, editing/revising

Our project repo can be found at on [Github (click me)](https://github.com/thecbp/FMPH221_project), commits can be reviewed through time. Alex had trouble working with Github and had to send files through email.
...
9. Bahk J, Jang SM, Jung-Choi K. Increased breast cancer mortality only in the lower education group: age-period-cohort effect in breast cancer mortality by educational level in South Korea, 1983-2012. Int J Equity Health. 2017;16(1):56. Published 2017 Mar 31. doi:10.1186/s12939-017-0554-6
10. https://nces.ed.gov/pubs93/93442.pdf
